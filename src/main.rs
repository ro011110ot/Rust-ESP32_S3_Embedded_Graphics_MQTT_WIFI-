...
unsafe impl GlobalAlloc for EspAlloc {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 { esp_alloc::HEAP.alloc(layout) }
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) { esp_alloc::HEAP.dealloc(ptr, layout) }
}
...
// ============================================================================
// Hardware Workarounds & Specifics
// ============================================================================
...
// ============================================================================
// Global Application State (Thread-Safe Wrapper)
// ============================================================================
...
impl AppState {
    ...
    pub fn set_wifi_connected(&self, v: bool) {
        critical_section::with(|cs| { self.inner.borrow_ref_mut(cs).wifi_connected = v; });
    }

    pub fn set_active_screen(&self, s: display::Screen) {
        critical_section::with(|cs| { self.inner.borrow_ref_mut(cs).active_screen = s; });
    }

    pub fn set_vps(&self, cpu: f32, ram: f32, disk: f32, uptime: u64) {
        critical_section::with(|cs| {
            let inner = &mut *self.inner.borrow_ref_mut(cs);
            inner.vps = Some(VpsData { cpu_pct: cpu, ram_pct: ram, disk_pct: disk, uptime_secs: uptime });
        });
    }

    pub fn set_host(&self, cpu: [f32; 4], cpu_temp: f32, ram: f32, ssd_temp: f32, net_down: f32) {
        critical_section::with(|cs| {
            let inner = &mut *self.inner.borrow_ref_mut(cs);
            inner.host = Some(HostData { cpu, cpu_temp, ram_pct: ram, ssd_temp, net_down });
        });
    }
    ...
    pub fn set_local_time(&self, t: time::LocalTime) {
        critical_section::with(|cs| { self.inner.borrow_ref_mut(cs).local_time = Some(t); });
    }
    ...
    pub fn set_weather(&self, temp: f32, humidity: f32, wind: f32, pressure: f32, desc: &str, icon: &str) {
        critical_section::with(|cs| {
            let inner = &mut *self.inner.borrow_ref_mut(cs);
...
async fn led_task() {
    ...
    unsafe {
        ...
        let _reset = Output::new(periphs.GPIO48, Level::Low, OutputConfig::default());
        Timer::after(Duration::from_millis(1)).await;
    }

    let rmt = esp_hal::rmt::Rmt::new(p.RMT, esp_hal::time::Rate::from_mhz(80)).unwrap();
    let mut led = Rgb8RmtSmartLeds::<{ buffer_size::<RGB8>(1) }, _, color_order::Grb, Ws2812FixedTiming>::new(rmt.channel0, p.GPIO48).unwrap();
...
#[embassy_executor::task]
async fn net_runner_task(mut runner: embassy_net::Runner<'static, esp_radio::wifi::Interface<'static>>) {
    runner.run().await
}
...
async fn main(spawner: Spawner) -> ! {
    unsafe {
        ...
        esp_alloc::HEAP.add_region(HeapRegion::new(
            core::ptr::addr_of_mut!(HEAP_BUF).cast::<u8>(), HEAP_SIZE, MemoryCapability::Internal.into(),
        ));
        esp_alloc::HEAP.add_region(HeapRegion::new(
            0x3FCDB700usize as *mut u8, 0x3FCED710usize - 0x3FCDB700usize, MemoryCapability::Internal.into(),
        ));
    ...
    defmt::info!("System: ESP32-S3 initialised");
    ...
    let (wifi_ctrl, interfaces) = esp_radio::wifi::new(peripherals.WIFI, controller_config).expect("esp-radio wifi init failed");

    static mut RESOURCES: embassy_net::StackResources<4> = embassy_net::StackResources::<4>::new();
    let (stack, runner) = embassy_net::new(
        interfaces.station, embassy_net::Config::dhcpv4(Default::default()),
        unsafe { &mut *core::ptr::addr_of_mut!(RESOURCES) }, rng_seed,
    );
    ...
    defmt::info!("System: all tasks spawned");
